...
async function setImagesSettingIfNeeded(setting) {
  ...
  if (currentSetting !== setting) {
    ...
    console.log(
      `Images setting is already ${
        setting === "allow" ? "allowed" : "blocked"
      }. No change needed.`
    );
  }
}
...
async function applySavedImagesSetting() {
  chrome.storage.sync.get(["imagesSetting", "interval"], async (result) => {
    ...
    if (result.interval !== 0 && setting === "allow") {
      console.log(
        "Interval is not zero and images are allowed, setting the timer"
      );
      setAlarm();
    }
  });
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  ...
  } else if (request.action === "restartAlarm") {
    try {
      resetAlarm();
      sendResponse({ status: "done" });
    } catch (error) {
